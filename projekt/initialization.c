...
int readFromFile(int* x, int* y, int* antCounter, int* heading, int boardSize, int* handlingLenght, char* handling, char* fileARG) // funkcja czytająca z pliku
{
	...
	for( i = 0;i < (*antCounter); i++){   // iterujemy się po wszytskich mrówkach i pokolei wczytujemy parametry startowe
//        fscanf(file, "%d", &x[i]);
//        fscanf(file, "%d", &y[i]);
//        fscanf(file, "%d", &heading[i]);
        fscanf(file, "%d %d %d", &x[i], &y[i], &heading[i]);
	}
